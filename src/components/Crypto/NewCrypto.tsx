...
const NewCrypto = () => {
    ...
    const [fetchCryptoPrice, {loading}] = useLazyQuery<CryptoToEuroData,
        CryptoToEuroVariables>(GET_CONVERSION_RATE, {
        fetchPolicy: "no-cache",
    });

    const codeInputRef = useRef<HTMLInputElement>(null);

    const resetInput = () => codeInputRef.current!.value = "";

    const isInputEmpty = (code: string) => code.trim().length === 0;

    const getCodeInputValue = () => codeInputRef.current!.value.toUpperCase();

    const handleSubmit = async (event: SyntheticEvent) => {
        event.preventDefault();
        setError("");

        const enteredCode = getCodeInputValue();

        if (isInputEmpty(enteredCode)) return setError("The input is empty");
    ...
    return (
        <div
            className="w-full md:max-w-sm bg-white border border-gray-200 rounded-md shadow p-10 sm:p-10 md:p-10 md:col-start-8 lg:col-start-3 self-baseline z-10 mt-6 md:mt-0 col-span-1 lg:col-span-1 md:col-span-5">
